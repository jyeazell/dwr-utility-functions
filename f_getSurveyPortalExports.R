...
# getReportingComplianceCSV <- function(rept_year = 2019,
                                       cred_file = "survey_portal_creds.csv"#,
                                       save_file = TRUE#) {
    ...
    remDr$navigate("https://public.waterboards.ca.gov/WRInfo/Account/Login?")

    # Enter login credentials.
    username <- remDr$findElement(using = "name", value = "Username")
    ...
    # Navigate to datafile export page.
    remDr$navigate("https://public.waterboards.ca.gov/WRInfo/Admin/Export")
    
    # Target survey selection frame.
    # iframe <- remDr$findElement(using="name", value="InfoOrderId")
    ...
    # dd_string <- '<option value="40">Request For Additional Time</option>'
